  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios({
        method: "post",
        url: `${import.meta.env.VITE_URL}/news?name=${data.name}&description=${data.description}&workId=${selectWork}&category=${selectCategory}`,
        data: formData,
              ...
              >
                {
                  categories && categories.length ?
                    categories.map((e) => {
                      return <MenuItem key={e.id} value={e.name}>{e.name}</MenuItem>
                    }) : <MenuItem value='No hay categoria'>No hay categoria</MenuItem>
                }
              </Select>
